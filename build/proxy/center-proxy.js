...
class CenterProxy {
  ...
  start() {
    ...
    this.proxy.on('request', this.requestHandler).on('connect', this.connectHandler).listen(httpPort);

    this.secureProxy.on('request', this.requestHandler).listen(httpsPort);
  }
